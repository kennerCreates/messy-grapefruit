...
│   ├── stores/              (project.ts, editor.ts, palette.ts, animation.ts, skins.ts, preferences.ts)
│   ├── engine/
...
│   │   ├── panels/          (LayerPanel, PalettePanel, AnimationPanel, SkinPanel)
...
| `export_sprite(sprite, animation, output_dir, settings)` | Export one animation → spritesheet or runtime bone data. Bone mode exports all skins (one atlas per skin, shared animation RON) |
| `export_all(project, sprites, settings)` | Export all sprites' animations (all skins) |
